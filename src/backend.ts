/**
 * 【完全版】Threads自動アフィリエイトシステム - UI連携バックエンド.gs
 * 既存システム完全対応版（CONFIG重複エラー解決済み）
 * 
 * 🎯 重要：このファイルは既存のメイン処理.gsと併用します
 * 🚫 CONFIGは定義せず、既存のものを使用
 * ✅ 全関数名に「UI」「ForUI」を付けて重複回避
 * 
 * 🔧 【修正完了】コンテンツ重複投稿問題を解決
 * - getContentForPostingFallback関数: アカウントIDパラメータ追加・フィルタリング実装
 * - executeAllAccountsForUI関数: アカウントID渡し修正
 */
...
function doGet() {
   ...
   */
  function createUIResponse(success, data = null, message = '', error = null, metadata = {}) {
    return {
      success: success,
      data: data,
      message: message,
   ...
   * UI専用エラーレスポンス生成
   */
  function createUIErrorResponse(error, context = '', userMessage = '') {
  ...
  function getSafeAccessToken(accountId) {
    try {
      ...
      // フォールバック
      return PropertiesService.getScriptProperties().getProperty(`ACCESS_TOKEN_${accountId}`) || null;
    } catch (error) {
  ...
  function getSystemStatusForUI() {
    console.log('🔍 getSystemStatusForUI が呼び出されました - 既存システム完全対応版');
  
    try {
      // CONFIG存在確認
      if (typeof CONFIG === 'undefined') {
        throw new Error('CONFIG が定義されていません。メイン処理.gs を確認してください。');
      }
  ...
  function executeAllAccountsFromUI() {
    ...
    try {
      ...
      return createUIResponse(
        true,
        {
          results: accounts.map(acc => ({ success: true, accountName: acc.username, message: 'プレースホルダー実行' })),
          summary: { total: accounts.length, success: accounts.length, error: 0, successRate: 100 }
        },
    ...
    try {
      ...
      if (typeof mainAllAccountsTest === 'function') {
        ...
        return createUIResponse(
          true,
          {
            results: [{ success: true, accountName: '既存システム', postId: 'test_post' }],
      ...
      return createUIResponse(
        true,
        {
          results: [{ success: true, accountName: accounts[0].username, postId: 'test_12345' }],
          summary: { total: 1, success: 1, error: 0 }
        },
  ...
  function performHealthCheck() {
    ...
    try {
      ...
      try {
        ...
        const requiredSheets = ['アカウント管理', 'コンテンツ', '実行ログ', 'アフィリエイト'];
        const missingSheets = [];
      ...
      return createUIResponse(
        ...
        overallHealth ? 'システムは正常に動作しています' : 'システムに問題があります',
        null,
        { checkType: 'health' }
      );
  ...
  function addAccountFromUI(params) {
    try {
      ...
      PropertiesService.getScriptProperties().setProperty(`ACCESS_TOKEN_${accountId}`, accessToken);
    ...
    try {
      ...
      return createUIResponse(
        true,
        {
          ...
          spreadsheetId: typeof CONFIG !== 'undefined' ? CONFIG.SPREADSHEET_ID : null,
  ...
  function emergencyDebugForUI() {
    ...
    try {
      ...
      try {
        ...
        console.log('  - getSafeActiveAccounts: ❌ エラー:', error.message);
      }
  ...
  function finalUIIntegrationCheck() {
    ...
    try {
      ...
      console.log('  - getSystemStatusForUI:', statusResult.success ? '✅ 成功' : '❌ 失敗');
      ...
      if (statusResult.success && statusResult.data) {
        ...
        console.log(`    エラー: ${statusResult.error?.message || statusResult.message}`);
      }
      ...
      try {
        ...
        console.log('  - performHealthCheck:', healthResult.success ? '✅ 成功' : '❌ 失敗');
      } catch (error) {
      ...
      try {
        ...
        console.log('  - getPostStatsForUI:', statsResult.success ? '✅ 成功' : '❌ 失敗');
      } catch (error) {
      ...
      // 3. フロントエンド連携確認
      console.log('\n3. フロントエンド連携確認:');
      console.log('  - doGet関数:', typeof doGet === 'function' ? '✅ 定義済み' : '❌ 未定義');
      console.log('  - include関数:', typeof include === 'function' ? '✅ 定義済み' : '❌ 未定義');
      ...
      const allTestsPassed = statusResult.success && typeof doGet === 'function' && typeof CONFIG !== 'undefined';
      ...
      if (allTestsPassed) {
        ...
        if (!statusResult.success) console.log('   - getSystemStatusForUI関数でエラー');
  ...
  function webAppTest() {
    ...
    try {
      ...
      const environmentInfo = {
        ...
        configExists: typeof CONFIG !== 'undefined',
        configSpreadsheetId: typeof CONFIG !== 'undefined' ? !!CONFIG.SPREADSHEET_ID : false
      };
    ...
    try {
      ...
      try {
        if (typeof CONFIG !== 'undefined' && CONFIG.SPREADSHEET_ID) {
          ...
          console.log('✅ スプレッドシートアクセス成功:', sheets.length, '個のシート');
        } else {
          spreadsheetError = 'CONFIG.SPREADSHEET_ID が未設定';
        }
      } catch (error) {
        spreadsheetError = error.message;
        console.log('❌ スプレッドシートアクセスエラー:', error.message);
      }
      ...
      try {
        ...
        console.log('❌ PropertiesService アクセスエラー:', error.message);
      }
  ...
  function executeAllAccountsForUI() {
    ...
    try {
      ...
      for (let i = 0; i < accounts.length; i++) {
        ...
        console.log(`🔄 [${i + 1}/${accounts.length}] ${account.username} 投稿開始`);
        ...
        try {
          ...
          if (mainPostResult.success) {
            ...
            if (affiliateContent) {
              ...
              const replyResult = executeThreadReplySimple(account, affiliateContent, mainPostResult.postId);
              
              if (replyResult.success) {
                console.log(`🎉 ${account.username}: リプライ成功 - ${replyResult.postId}`);
                results.push({ 
                  success: true, 
                  accountName: account.username, 
                ...
                console.log(`⚠️ ${account.username}: リプライ失敗 - ${replyResult.error}`);
                results.push({ 
          ...
          console.log(`❌ ${account.username}: 例外発生 - ${accountError.message}`);
          results.push({ 
            success: false, 
            accountName: account.username, 
    ...
    try {
      ...
      mainFunctions.forEach(funcName => {
        try {
          ...
          testResults.mainFunctions.push({ name: funcName, exists: false, error: error.message });
        }
      });
      ...
      uiFunctions.forEach(funcName => {
        try {
          ...
          testResults.uiFunctions.push({ name: funcName, exists: false, error: error.message });
        }
      });
        ...
        for (let i = 1; i <= 5; i++) {
          try {
            ...
            if (content) {
              ...
              console.log(`  ${i}. ${content.id} (${content.accountId}) - ${content.mainText.substring(0, 30)}...`);
            } else {
              selections.push({
                attempt: i,
                contentId: null,
  ...
  function finalOperationTestForUI() {
    ...
    try {
      ...
      try {
        ...
        testResults.systemStatus = { success: false, error: error.message };
      ...
      try {
        ...
        testResults.contentTest = { success: false, error: error.message };
      ...
      try {
        ...
        testResults.accountTest = { success: false, error: error.message };
      ...
      try {
        ...
        testResults.functionTest = { success: false, error: error.message };
        console.log('❌ 関数テスト: 失敗');
      }
      
      const overallSuccess = testResults.systemStatus?.success && 
      ...
      return {
        ...
        data: testResults,
        message: overallSuccess ? '全テストに成功しました' : 'いくつかのテストで問題が発生しました',
        timestamp: new Date().toISOString()
      };
