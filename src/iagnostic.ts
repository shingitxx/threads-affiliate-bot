import { CONFIG, handleUnknownError, HTTP_METHODS } from './utils';
...
function emergencyDiagnosisForUI() {
    ...
    try {
        ...
        if (!results.mainFunctionExists) {
            console.log(
                '❌ 関数未定義 → UI連携バックエンド.gsが正しく保存されているか確認'
            );
        }
        if (
            results.configExists &&
            results.mainFunctionExists &&
            !results.spreadsheetAccess
        ) {
            console.log(
                '❌ スプレッドシート問題 → SPREADSHEET_IDとアクセス権限を確認'
            );
        }
        ...
        results.errors.push(`診断エラー: ${handleUnknownError(error)}`);
        return results;
    }
}
...
function testGetSystemStatusForUIManually() {
    ...
    try {
        ...
        if (typeof getSystemStatusForUI !== 'function') {
            ...
            return {
                success: false,
                error: 'getSystemStatusForUI関数が存在しません',
            };
        }
        ...
        console.log(
            '  - success プロパティ:',
            result && typeof result.success !== 'undefined'
                ? result.success
                : 'undefined'
        );
        console.log(
            '  - data プロパティ:',
            result && typeof result.data !== 'undefined' ? result.data : 'undefined'
        );
        console.log(
            '  - message プロパティ:',
            result && result.message ? result.message : 'undefined'
        );
...
function diagnoseAccountStatus() {
    ...
    try {
        ...
        accounts.forEach((account) => {
            ...
            console.log(
                `  🔑 トークン状態: ${account.accessToken ? '✅ 設定済み' : '❌ 未設定'}`
            );
            ...
            if (account.accessToken) {
                ...
                console.log(
                    `  🔑 トークン先頭: ${account.accessToken.substring(0, 20)}...`
                );
            }
            ...
            if (content) {
                ...
                console.log(
                    `  📝 コンテンツ内容: ${content.mainText.substring(0, 30)}...`
                );
            } else {
    ...
    try {
        ...
        if (relevantLogs.length === 0) {
            ...
            recentLogs.forEach((row) => {
                console.log(
                    `  ${new Date(row[0]).toLocaleString()} | ${row[1]} | ${row[2]} | ${row[4]}`
                );
            });
        } else {
        ...
        Object.keys(accountResults).forEach((account) => {
            ...
            console.log(
                `👤 ${account}: 成功${results.success}件, 失敗${results.failure}件`
            );
        });
    } catch (error) {
        ...
        accounts.forEach((account) => {
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                if (content) {
                    ...
                    console.log(
                        `  ${i}. ✅ ${content.id}: ${content.mainText.substring(0, 30)}...`
                    );

                    // 対応するアフィリエイトテスト
                    const affiliate = getRandomAffiliateForAccount(
                        content.id,
                        account.id
                    );
                    if (affiliate) {
                        console.log(
                            `    🔗 アフィリエイト: ${affiliate.id} - ${affiliate.description.substring(0, 30)}...`
                        );
                    } else {
            ...
            if (contentSuccess === 0) {
                console.log(
                    `  ⚠️ ${account.username}: 投稿可能なコンテンツがありません`
                );
                console.log(
                    `  💡 対策: ${account.id} 用のコンテンツを追加してください`
                );
            }
        });
    } catch (error) {
...
function runComprehensiveDiagnosis() {
    ...
    try {
        ...
        console.log('\n🎯 === 診断完了 ===');
        console.log(
            '📊 上記の結果を確認して、ACC001の投稿失敗原因を特定してください'
        );
