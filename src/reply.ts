 ...
 * リプライ完了マーク
 */
function markReplyAsCompleted(replyId: number): void {
    try {
    const sheet = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID).getSheetByName(
      CONFIG.SHEET_NAMES.SCHEDULE
    );
 ...
 */
function getAffiliateContentFromReply(contentId: string) {
    try {
    const sheet = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID).getSheetByName(
      CONFIG.SHEET_NAMES.AFFILIATE
    );
 ...
 */
function executeThreadReplyFromReplyFile(
    account: Account,
...
function logAffiliateActivity(account, affiliateContent, postId) {
  try {
    ...
    console.log(
      `アフィリエイト活動をログしました: ${affiliateContent.appName}`
    );
  } catch (error) {
...
function generateAffiliateReport() {
  try {
    ...
    const report = {
      ...
      totalConversions: thisMonthData.reduce(
        (sum, row) => sum + (row[6] || 0),
        0
      ),
      conversionRate: 0,
    ...
    if (report.totalClicks > 0) {
      report.conversionRate = (
        (report.totalConversions / report.totalClicks) *
        100
      ).toFixed(2);
    }
      ...
      if (trigger.getHandlerFunction() === 'executeReplyPost') {
        const triggerTime =
          trigger.getTriggerSource() === ScriptApp.TriggerSource.CLOCK
            ? new Date(trigger.getTriggerSourceId())
            : null;
