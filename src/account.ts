 ...
 * 2. setupNewAccount() 関数にアクセストークンを設定
 * 3. 実行してアカウント追加完了
 * 
 * 作成日: 2024年12月21日
 * 更新日: 2024年12月21日
...
function setupNewAccount() {
    ...
    if (newAccountToken === 'YOUR_ACCESS_TOKEN_HERE') {
      ...
      console.log('👉 上記の const newAccountToken = の行にアクセストークンを貼り付け');
    ...
    try {
      ...
      // Step 3: スプレッドシートに追加
      console.log('\n📊 Step 3: スプレッドシートに追加...');
      const addResult = addAccountToSpreadsheetSafe(accountId, userInfo.username, userInfo.userId);
      ...
      console.log('\n🎉 === 新規アカウント追加完了! ===');
      console.log('🧪 次のコマンドでテスト投稿: testSpecificAccount("' + accountId + '")');
   ...
   * 汎用アカウントセットアップ関数
   */
  function setupAccountWithToken(accountId, accessToken) {
   ...
   * 安全なアカウント追加（重複チェック付き）
   */
  function addAccountToSpreadsheetSafe(accountId, username, userId) {
    try {
      const sheet = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID)
        .getSheetByName(CONFIG.SHEET_NAMES.ACCOUNTS);
      
      if (!sheet) {
        return { success: false, message: 'アカウント管理シートが見つかりません' };
      }
      ...
      if (lastRow > 1) {
        ...
        // アカウントID重複チェック
        const duplicateId = data.some(row => row[0] === accountId);
        if (duplicateId) {
          return { success: false, message: `アカウントID ${accountId} は既に存在します` };
        }
        
        // ユーザー名重複チェック
        const duplicateUsername = data.some(row => row[1] === username);
        if (duplicateUsername) {
          return { success: false, message: `ユーザー名 ${username} は既に存在します` };
        }
      }
   ...
   * アカウント設定確認
   */
  function verifyAccountSetup(accountId) {
    try {
      ...
      console.log(`✅ アクセストークン: ${targetAccount.accessToken ? '設定済み' : '未設定'}`);
   ...
   * 指定アカウントでテスト投稿
   */
  function testSpecificAccount(accountId) {
   ...
   * トラブルシューティング表示
   */
  function displayTroubleShooting(responseCode) {
    console.log('\n🔧 === トラブルシューティング ===');
    ...
    switch (responseCode) {
      ...
      default:
        console.log(`❓ ${responseCode} 予期しないエラー`);
   ...
   * 手動設定手順表示
   */
  function displayManualInstructions(accountId, userInfo) {
    console.log('\n📋 === 手動設定手順 ===');
   ...
   * 保存済みトークン一覧表示
   */
  function listSavedTokens() {
    console.log('🔍 === 保存済みトークン一覧 ===');
    
    const properties = PropertiesService.getScriptProperties().getProperties();
    const tokenKeys = Object.keys(properties).filter(key => key.startsWith('TOKEN_'));
   ...
   * 特定アカウントのトークン削除
   */
  function removeAccountToken(accountId) {
    try {
      PropertiesService.getScriptProperties().deleteProperty(`TOKEN_${accountId}`);
      console.log(`✅ ${accountId} のトークンを削除しました`);
    } catch (error) {
      console.error(`❌ ${accountId} のトークン削除エラー:`, error);
    }
  }
  ...
  function bulkSetupAccounts() {
    ...
    const accountsToAdd = [
      ...
      {
        id: 'ACCOUNT_010',
        token: 'THAAkIds0IIlZABUVNBN3NtcTRoSnJ4NF9yQklLRmpwRVQwSTRicEtzeHc1cmpIdUNGeVk3aDFfV0l0NWFfSmphWHBRT3FtOTRHeTlmNnVVeERZAT0lBNWtQXzdTTzd3VkVZAS2NYclpZAdk8xLW84MVZAfcUxPS0FLTl83S2h1S0o4cmxUQjRGN280Uk5sNllPbkEZD'
      },
      {
        id: 'ACCOUNT_011',
    ...
    accountsToAdd.forEach((account, index) => {
      console.log(`\n🔄 ${index + 1}/${accountsToAdd.length}: ${account.id} 処理中...`);
      
      if (account.token === 'YOUR_TOKEN_3_HERE' || 
    ...
    console.log(`📈 成功率: ${successCount + failCount > 0 ? Math.round(successCount / (successCount + failCount) * 100) : 0}%`);
  }